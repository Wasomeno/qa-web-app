...
export const ProjectBoardView: React.FC<ProjectBoardViewProps> = ({
    ...
    (event: DragOverEvent) => {
      ...
      setColumns((prev) => {
        ...
        const activeColIdx = prev.findIndex(
          (col) =>
            col.id === activeId || col.issues.some((i) => i.id === activeId),
        );
  ...
  const persistMoveOptimistically = useCallback(
    async (
      issueId: string,
      sourceColId: string,
      targetColId: string,
  ...
  const handleDragEnd = useCallback(
    (event: DragEndEvent) => {
      ...
      if (activeColumn.id === overColumn.id) {
        ...
        const overIndex = activeColumn.issues.findIndex(
          (i) => i.id === over.id,
        );
        ...
        <div className="flex flex-1 gap-4 px-6 py-4 min-w-min">
          {columns.map((column) => (
            <DroppableColumn
              ...
              column={column}
              isDropTarget={
                overColumnId === column.id && overColumnId !== startColumnId
              }
            >
              <SortableContext
